/* ==========================================================================
   AURORA JOURNALS - CORE ENGINE (SINGLE PRODUCT CATALOG SETUP)
   ========================================================================== */
...
function initializeCatalogEngine() {
    ...
    if (searchField) {
        searchField.addEventListener("input", (e) => {
            renderCatalogDisplay(e.target.value.trim());
        });
    }
    
    // Auto-Routing: If on single product details page, dynamically render profile
    if (window.location.pathname.includes("product.html")) {
        renderProductProfileDisplay();
    }
}

function renderCatalogDisplay(filterQuery = "") {
...
function renderProductProfileDisplay() {
    const detailSurface = document.getElementById("product-detail-surface");
    if (!detailSurface) return;

    const targetItem = PRODUCTS_DB[0]; // Force default single inventory anchor
...
function executeAddToCart(productId) {
    ...
    // Fire auto-open drawer animation sequence
    document.getElementById("cart-global-overlay").classList.add("open");
    document.getElementById("cart-global-drawer").classList.add("open");
}
